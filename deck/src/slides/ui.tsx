...
export const BarRow: React.FC<{ label: string; value: string; pct: number; tone?: "blue" | "warm"; delay?: number }> = ({
  ...
  return (
    <div className="mb-[3.2%]">
      ...
      <div className="h-[11px] rounded-full bg-white/10 overflow-hidden">
        <div
          ...
          style={{
            ...
            background: tone === "warm"
              ? "linear-gradient(90deg,#c75f17,#ffc488)"
              : "linear-gradient(90deg,#1f4ba0,#7fb0ff)",
          }}
        />
      </div>
    </div>
...
export const Chromo: React.FC = () => {
  const A = "#7fb0ff", Al = "rgba(127,176,255,0.3)", Gl = "rgba(255,255,255,0.12)", G = "rgba(255,255,255,0.4)", O = "#ffc488";
  ...
  const Row = ([label, cells, bold]: [string, string[], boolean]) => (
    <div key={label} className="flex items-center gap-[12px] my-[7px]">
      <div className="mono shrink-0" style={{ width: 58, fontSize: 12, fontWeight: bold ? 700 : 500, color: bold ? "#7fb0ff" : "rgba(255,255,255,0.6)" }}>{label}</div>
      <div className="flex gap-[6px]">
        {cells.map((c, i) => (
          <div key={i} style={{ width: 24, height: 24, borderRadius: 6, background: c, boxShadow: c === O ? "0 0 0 2px rgba(255,196,136,0.4)" : "none" }} />
        ))}
      </div>
    </div>
  );
  return (
    <div>
      <div className="mono text-white/45 tracking-[0.08em] mb-[10px]" style={{ fontSize: 11.5 }}>CROSSOVER &amp; MUTATION ON A CHROMOSOME</div>
      {Row(rows[0])}
      {Row(rows[1])}
      <div className="mono text-white/45 my-[8px] ml-[70px]" style={{ fontSize: 12 }}>↓ crossover at midpoint · then one mutation</div>
      {Row(rows[2])}
      <div className="mono flex gap-[18px] mt-[13px] ml-[70px] text-white/60" style={{ fontSize: 11.5 }}>
